...
if (empty($arquivo) && empty($nome) && empty($endereco)) {
    ...
    header("Refresh: 4; url = http://localhost/Trabalho_final/form/form_ponto_turistico.php");
} else {
    ...
    //O arquivo passou em todas as verificações, hora de tentar move-lo para a pasta foto
        ...
        if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta'] . $nome_final)) {
            ...
            $ps = $pdo->prepare('INSERT INTO ponto_turistico (nome, endereco, imagem) VALUES (?,?,?);');
            if ($ps->execute([$nome, $endereco, $nome_final])) {
